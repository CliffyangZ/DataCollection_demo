...
def start_collection():
    ...
    try:
        ...
        if not os.path.isfile(config_path):
            ...
            app.logger.info(f"嘗試尋找已知的配置文件...")
            
            # 嘗試使用相對路徑尋找已知的配置文件
            local_config_path = os.path.join(project_root, 'backend', 'api_config', 'BingX_api_config2_local.json')
            
            if os.path.isfile(local_config_path):
                app.logger.info(f"找到替代配置文件: {local_config_path}")
                config_path = local_config_path
            else:
                return jsonify({
                    "success": False,
                    "message": f"配置文件不存在且無法找到替代: {config_path}"
                }), 400
...
def stop_collection():
    ...
    # 如果收集狀態顯示為非收集中，但前端仍然嘗試停止，可能是進程已自然完成
    if not collection_status["isCollecting"]:
        # 確保監控線程已停止
    ...
    # 如果進程對象不存在，但狀態顯示為收集中，修正狀態
    if collection_status["isCollecting"] and not collection_process:
        collection_status["isCollecting"] = False
        should_stop_monitoring = True
        if log_monitor_thread and log_monitor_thread.is_alive():
            log_monitor_thread.join(timeout=5)
    ...
    try:
                ...
                "configFiles": [
                    {
                        ...
                        "path": os.path.abspath(os.path.join(config_dir, "BingX_api_config2_local.json")),
                        "exchange": "BingX"
                    },
                    {
                        "filename": "Binance_api_config.json",
                        "path": os.path.abspath(os.path.join(config_dir, "Binance_api_config.json")),
                        "exchange": "Binance"
                    },
                    {
                        "filename": "OKX_api_config.json",
                        "path": os.path.abspath(os.path.join(config_dir, "OKX_api_config.json")),
                        "exchange": "OKX"
                    },
                    {
                        "filename": "ByBit_api_config.json",
                        "path": os.path.abspath(os.path.join(config_dir, "ByBit_api_config.json")),
                        "exchange": "ByBit"
                    }
                ],
        ...
        for config in configs:
            if not os.path.isabs(config['path']):
                config['path'] = os.path.abspath(os.path.join(project_root, config['path'].lstrip('/')))
        
        app.logger.info(f"返回帶絕對路徑的配置文件: {configs}")
...
if __name__ == '__main__':
    ...
    # 添加命令行參數支持
    import argparse
    parser = argparse.ArgumentParser(description='數據收集API服務')
